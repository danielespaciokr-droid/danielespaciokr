...
- **글자를 찾아서 맞추기**: 사진마다 오른쪽 아래의 영문 글자(크레딧, 워터마크)를 직접 찾아서, 그 글자 둘레를 지웁니다. 사진 크기와 방향이 제각각이어도 글자를 따라갑니다. ([아래](#사진마다-글자를-찾아서-맞추기) 참고)
...
- **글자 찾기**: 사진마다 오른쪽 아래의 글자를 찾아 공통 영역을 그 글자에 맞춰 옮깁니다. 공통 영역을 저장할 때 영역 안의 글자 위치를 기억해 두므로, 글자 둘레의 여백도 그린 그대로 따라갑니다.
...
5. 저장할 때 크기가 다른 사진에서 영역을 어디에 놓을지 고릅니다.
   - **기준 위치에 맞추기** (기본값): 영역이 가까운 모서리(예: 오른쪽 아래)에서 떨어진 거리를 지킵니다. 해상도가 조금 다른 사진에서도 같은 모서리에 놓이므로 글씨, 워터마크, 날짜에 알맞습니다. 기준 위치는 그린 자리를 보고 자동으로 정해지고, 바꿀 수도 있습니다.
   - **사진 크기에 비례해서 늘리기**: 가로와 세로를 사진 크기에 맞춰 각각 늘리거나 줄입니다.
   - **크기나 가로세로 비율이 같은 사진에만 쓰기**: 다른 비율의 사진은 처리하지 않고 오류로 알려 줍니다.

다른 사진을 연 뒤 공통 영역을 고르고 **[이 사진에 적용]** 을 누르면, 그 사진의 어디에 놓이는지 미리 볼 수 있습니다. **[글자 찾아 적용]** 을 누르면 그 사진의 글자를 찾아서 맞춘 자리를 보여 줍니다 (폴더 자동 처리가 하는 것과 같습니다).

저장할 때 영역 안의 글자 위치도 함께 기억합니다 (자동 처리 창에 "글자 위치 기억함"이 보입니다). 이 기능이 생기기 전에 저장한 공통 영역은, 가로 사진과 세로 사진을 한 장씩 열어 [이 사진에 적용] → [지금 영역 저장...]으로 **같은 이름으로 한 번씩 다시 저장** 하면 더 정확해집니다.
...
5. **"사진마다 오른쪽 아래 글자를 찾아서 영역을 맞추기 (추천)"** 는 켜 둡니다 (기본값). 사진마다 글자를 찾아 그 자리에 영역을 옮기고, 못 찾으면 저장된 위치를 지웁니다.
...
공통 영역은 사진 크기에 맞춰 놓이지만, 사진마다 해상도나 비율이 다르면 글자와 조금씩 어긋날 수 있습니다. 그래서 사진마다 **오른쪽 아래의 영문 글자를 직접 찾아서** 영역을 그 글자에 맞춥니다.
...
| 글자와 조금 어긋난 곳이 지워짐 | 자동 처리 창의 "사진마다 오른쪽 아래 글자를 찾아서 영역을 맞추기"가 켜져 있는지 보세요. 공통 영역을 가로·세로 사진에서 한 번씩 다시 저장하면 글자 위치를 기억해서 더 정확해집니다. 메인 창의 [글자 찾아 적용]으로 사진마다 어디를 지울지 미리 볼 수 있습니다. |
| 기록에 "글자 못 찾음: 저장된 위치"가 자주 나옴 | 그 사진을 메인 창에서 열고 [글자 찾아 적용]을 눌러 보세요. 글자가 배경과 거의 같은 색이면 찾지 못합니다. 그런 사진이 계속 나오면 사진을 캡처해서 보내 주세요. |
