...
- **폴더 자동 처리**: 사진 폴더를 지켜보다가 사진이 새로 저장되면 1초 안팎에 알아서 정해진 영역을 선택해 지우고 저장합니다. 가로 사진과 세로 사진에 각각 맞는 영역을 씁니다. ([아래](#2-폴더-자동-처리-사진이-들어오면-바로-지우기) 참고)
...
- **폴더 자동 처리**: 사진 폴더를 지켜보다가 새 사진이 저장되는 대로 공통 영역을 선택해 지우고 저장합니다. 이미 지운 사진은 건너뛰고, 복사 중인 파일은 복사가 끝난 뒤 처리합니다. 전용 실행 파일(`auto_windows.bat`, `auto_mac.command`)로 바로 시작하거나 Windows를 켤 때 저절로 시작하게 할 수 있습니다.
...
4. **[▶ 시작]** 을 누릅니다. 처음에는 폴더에 이미 있는 사진을 처리하고, 그다음부터는 창을 열어 두는 동안 사진 폴더를 계속 지켜보다가 **새 사진이 저장되면 1초 안팎에** 공통 영역을 선택해 지우고 저장합니다. Photoshop이 꺼져 있으면 알아서 실행합니다.
...
- 한 장이 실패해도 나머지는 계속 처리하고, 실패한 사진은 기록에 이유와 함께 남습니다. Photoshop이 잠깐 응답하지 않으면 조금 뒤에 다시 시도합니다.
